...
bool ModeldComponent::Init()
{
  ...
  model_ = std::shared_ptr<ModelState>();
  // camera init
  model_init(model_, device_id_, context_);
  AINFO << "models loaded, modeld starting";

  vipc_client_ = VisionIpcClient("camerad", wide_camera_ ? VISION_STREAM_YUV_WIDE : VISION_STREAM_YUV_BACK, true, device_id, context);
...
void ModeldComponent::model_publish(uint32_t vipc_frame_id, uint32_t frame_id, float frame_drop,
  ...
  if (send_raw_pred) {
    framed->set_raw_predictions(&(*raw_pred.begin()), raw_pred.size());
  }
