...
bool CalibrationComponent::Init()
{
  ...
  reader_ = node_->CreateReader<CameraOdometry>(
    "CameraOdometry", [this](const std::shared_ptr<CameraOdometry>& cam_odometry){
        std::lock_guard<std::mutex> lock(mutex_);
        cam_odometry_->CopyFrom(*cam_odometry);
    }
  );
  // read saved calibration
  calibration_params_ = calRecordReader("perception/calibration_component/CalibrationParams");
...
bool CalibrationComponent::Proc(const std::shared_ptr<CarState>& car_state)
{
  ...
  if(cam_odometry_ != nullptr && cam_odometry_->error_code() == apollo::common::ErrorCode::OK){
    ...
    auto new_rpy = handleCamOdom(cam_odometry_);
...
void CalibrationComponent::updateStatus()
{
  ...
  if(calib_spread_.maxCoeff() > MAX_ALLOWED_SPREAD && cal_status_ == Calibration::CALIBRATED){
  ...
  if(param_put_ && write_this_cycle){
    calRecordWriter("perception/calibration_component/CalibrationParams", getMsg().get());
  }
}
    ...
    const std::shared_ptr<CameraOdometry>& cam_odom)
{
  ...
  Eigen::Vector3d observed_rpy(0,
                               ...
                               std::atan2(cam_odom->trans_std(1), cam_odom->trans(0)));

  Eigen::Vector3d new_rpy = rot2euler(euler2rot(smooth_from_) * euler2rot(observed_rpy));
  new_rpy = sanityClip(new_rpy);

  rpys_.block(block_idx_, 0, BLOCK_SIZE, 3) = (idx_ * rpy_.block(block_idx_, 0, BLOCK_SIZE, 3)) + 
